...
class ChaoticSpeakerRecognitionNetwork(nn.Module):
    ...
    def __init__(
        ...
        use_raw: bool = False,
        use_differentiable_raw: bool = True,
        # ================
        ...
        self.use_differentiable_raw = use_differentiable_raw
        # ====================
        ...
        if self.use_raw:
            ...
            print("[INFO] Raw audio path DISABLED (matching checkpoint)")
        ...
        if self.include_mfcc:
            try:
                self.mfcc_transform = torchaudio.transforms.MFCC(
                    ...
                    n_mfcc=40,
                    melkwargs={'n_fft': frame_length, 'hop_length': hop_length, 'n_mels': 64}
                ).to(self.device)
                # 投影到与混沌特征相同的维度 (117)
                self.mfcc_projection = nn.Linear(40, 117).to(self.device)
                self.mfcc_gate = nn.Parameter(torch.tensor(0.5)) # 融合权重
                print("[INFO] MFCC extraction enabled (torchaudio).")
            except Exception as e:
                print(f"[WARNING] MFCC init failed: {e}")
                self.include_mfcc = False

        self.speaker_embedding_dim = speaker_embedding_dim
        self.embedding_hidden_dims = embedding_hidden_dims or [512, 256, 128]
        # Loss functions
    ...
    def _initialize_components(self):
        ...
        # === 加上条件判断 ===
        if self.use_differentiable_raw:
            try:
                ...
                diff_output_dim = self.diff_chaos_features.output_dim  # 270
                expected_dim = self.mlsa_feature_dim + self.rqa_feature_dim  # 230
                
                if diff_output_dim != expected_dim:
                    self.diff_feat_projection = nn.Sequential(
                        nn.Linear(diff_output_dim, expected_dim),
                        nn.LayerNorm(expected_dim),
                        nn.ReLU()
                    )
            ...
            self.use_differentiable_features = False
            print("[INFO] Differentiable chaos features DISABLED (matching checkpoint)")
        # ====================
    ...
    def forward(self, audio, labels=None, return_intermediates=False, debug=False):
        ...
        if hasattr(self, 'raw_audio_encoder'):
            ...
            raw_feat = self.raw_audio_encoder(audio_input).squeeze(-1)
            raw_feat = self.raw_audio_projection(raw_feat)

        # 2. MFCC 特征 (新增！)
        mfcc_feat = None
        if self.include_mfcc and hasattr(self, 'mfcc_transform'):
            # audio: [B, T] -> mfcc: [B, n_mfcc, time]
            mfcc = self.mfcc_transform(audio)
            # Global Average Pooling: [B, n_mfcc]
            mfcc_mean = mfcc.mean(dim=2)
            mfcc_feat = self.mfcc_projection(mfcc_mean)

        # 3. 混沌特征 (GPU/CPU)
        # 我们需要 phase_space_data 用于后面的 intermediate 返回
        if hasattr(self, 'use_differentiable_features') and self.use_differentiable_features:
            # GPU Path
            phase_space_data = differentiable_phase_space_reconstruction(
                audio, self.embedding_dim, getattr(self, 'fixed_delay', 10), device
            )
            chaotic_features = self.diff_chaos_features(phase_space_data)
            # 维度修正
            expected_dim = self.mlsa_feature_dim + self.rqa_feature_dim
            if chaotic_features.shape[-1] != expected_dim:
                if not hasattr(self, '_diff_feat_projection'):
                    self._diff_feat_projection = nn.Linear(chaotic_features.shape[-1], expected_dim).to(device)
                chaotic_features = self._diff_feat_projection(chaotic_features)
        else:
            # CPU Path (Legacy)
        ...
        if hasattr(self.chaotic_embedding, 'forward'):
            ...
            traj_len = chaotic_trajectories.shape[1] // 3
            chaotic_trajectories = chaotic_trajectories.view(batch_size, traj_len, 3)
        ...
        if mfcc_feat is not None:
            beta = torch.sigmoid(self.mfcc_gate)
            # 加权叠加
            pooled_features = beta * pooled_features + (1 - beta) * mfcc_feat

        # 5. 后续分类
        speaker_embeddings = self.speaker_embedding(pooled_features)
        logits = self.classifier(speaker_embeddings, labels)

        # === 【关键修复】这里必须处理 return_intermediates ===
...
def differentiable_phase_space_reconstruction(audio, dim, tau, device):
    """
    完全可导的 GPU 相空间重构。
    输入 audio: [Batch, Time]
    输出 embedded: [Batch, Time_Steps, Dim]
    """
    batch_size, length = audio.shape
    
    # 计算有效长度
    valid_length = length - (dim - 1) * tau
    if valid_length <= 0:
        # 容错处理
        return torch.zeros(batch_size, 200, dim, device=device)
        
    # 使用切片构建每一维，这是 PyTorch 中最高效且保留梯度的方法
    cols = []
    for i in range(dim):
        # 每一列是音频的一个延时版本
        start = i * tau
        end = start + valid_length
        cols.append(audio[:, start:end])
        
    # 堆叠成相空间 [Batch, Time, Dim]
    embedded = torch.stack(cols, dim=2)
    
    # 显存优化：如果太长，进行均匀下采样到 200 点（与 CPU 版行为一致）
    # 注意：这里必须用索引操作，不能断开梯度
    if embedded.shape[1] > 200:
        indices = torch.linspace(0, embedded.shape[1]-1, 200, device=device).long()
...
def create_chaotic_speaker_network(config: Dict) -> ChaoticSpeakerRecognitionNetwork:
    """
    Factory function to create ChaoticSpeakerRecognitionNetwork from YAML config.
    Auto-detects nested config structure and maps to model arguments.
    """
    # 1. 尝试从 config 中提取 'model' 部分（如果存在）
    model_config = config.get('model', config)
    
    # 2. 准备基础参数 (从嵌套结构中提取并扁平化)
    params = {
        'num_speakers': model_config.get('num_speakers', 26),
        'sample_rate': config.get('audio', {}).get('sample_rate', 16000),
        
        # Phase space
        'embedding_dim': model_config.get('phase_space', {}).get('embedding_dim', 10),
        'delay_method': model_config.get('phase_space', {}).get('delay_method', 'autocorr'),
        
        # Chaotic features
        'mlsa_scales': model_config.get('chaotic_features', {}).get('mlsa', {}).get('scales', 5),
        'rqa_radius_ratio': model_config.get('chaotic_features', {}).get('rqa', {}).get('radius_ratio', 0.1),
        
        # Chaotic embedding
        'chaotic_system': model_config.get('chaotic_embedding', {}).get('system_type', 'lorenz'),
        'evolution_time': model_config.get('chaotic_embedding', {}).get('evolution_time', 0.5),
        'time_step': model_config.get('chaotic_embedding', {}).get('time_step', 0.01),
        
        # Classifier & Embedding
        'classifier_type': model_config.get('classifier', {}).get('type', 'cosine'),
        'speaker_embedding_dim': model_config.get('speaker_embedding', {}).get('dim', 256),
        'embedding_hidden_dims': model_config.get('speaker_embedding', {}).get('hidden_dims', [512, 256, 128]),
        'pooling_type': model_config.get('attractor_pooling', {}).get('type', 'comprehensive'),
        
        # Hardware
        'device': config.get('hardware', {}).get('device', 'cpu')
    }
    
    mfcc_enabled = model_config.get('include_mfcc', False)

    # 3. 关键修复：读取 Optional Modules 决定是否开启 GPU 270维模式
    opt_modules = config.get('optional_modules', {})
    
    # 检查是否开启了 differentiable_features
    use_diff = opt_modules.get('differentiable_features', {}).get('enabled', False)
    
    params['use_differentiable_raw'] = use_diff
    params['use_raw'] = use_diff  # 开启 GPU 特征时，通常也开启 Raw Audio 路径
    params['include_mfcc'] = mfcc_enabled
    params['use_bifurcation_control'] = opt_modules.get('bifurcation_control', {}).get('enabled', False)

    print(f"\n[Factory] Creating network from YAML:")
    print(f"  - Use Differentiable Raw (GPU): {params['use_differentiable_raw']}")
    print(f"  - Target Dimension: {'270 (Hybrid)' if use_diff else '230 (Legacy)'}")
    
    return ChaoticSpeakerRecognitionNetwork(**params)
