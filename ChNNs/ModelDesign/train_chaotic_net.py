...
class ChaoticTrainer:
    ...
    def _load_data(self):
        ...
        val_meta = self.metadata[self.metadata['folder_subset'].isin(val_subsets)]
    ...
    def train(self):
        ...
        for epoch in range(self.config['epochs']):
            ...
            print("当前学习率:", self.scheduler.get_last_lr())
    ...
    def add_new_speakers(self, new_subset='dev-clean', sample_size=500):
        ...
        self.ewc = ElasticWeightConsolidation(
            model=self.model,
            dataset=self.train_set,  # 注意: 实际应使用旧任务数据
            device=self.device,
...
def get_default_config():
    """获取默认训练配置"""
    return {
        'data_root': 'P:/PycharmProjects/pythonProject1/dataset/LibriSpeech_processed',  # 预处理数据路径
