...
class Config:
    ...
    BASE_DIR = "P:/"
    PROJECT_DIR = os.path.join(BASE_DIR, "PycharmProjects", "pythonProject1")
    LIBRISPEECH_PATH = os.path.join(PROJECT_DIR, "dataset", "LibriSpeech")
    CHIME6_PATH = os.path.join(PROJECT_DIR, "devDataset", "CHiME6_dev", "CHiME6", "audio", "dev")
    RAVDESS_PATH = os.path.join(PROJECT_DIR, "devDataset", "ravdess")

    # 音频处理参数
    SAMPLE_RATE = 16000
    DURATION = 1.0  # 减少为1秒音频（16000个采样点）
    MAX_SAMPLES = int(SAMPLE_RATE * DURATION)

    # 噪声参数
    NOISE_TYPES = ["white", "babble"]
    SNR_LEVELS = [0, 5, 10]  # 减少SNR级别
...
class NoiseInjector:
    ...
    @staticmethod
    def generate_babble_noise(length, num_speakers=3):  # 减少说话人数
...
class SpeakerRecognitionDataset(Dataset):
    ...
    def _validate_path(self):
        ...
        if not os.path.exists(path):
            print(f"错误: {self.dataset_name} 路径不存在 - {path}")
            print("请确保数据集已下载并放置在正确位置")

            # 提供下载建议
            if self.dataset_name == "librispeech":
                print("LibriSpeech可以从 https://www.openslr.org/12 下载")
            elif self.dataset_name == "chime6":
                print("CHiME-6可以从 https://chimechallenge.github.io/chime6/download 下载")
            elif self.dataset_name == "ravdess":
                print("RAVDESS可以从 https://zenodo.org/record/1188976 下载")
    ...
    def _load_dataset(self):
        ...
        if self.dataset_name == "librispeech":
            ...
            # 遍历所有子集目录 (train-clean-100, train-clean-360, dev-clean等)
                    ...
                    for chapter_dir in os.listdir(speaker_path):
                        ...
                        for file in os.listdir(chapter_path):
                            if file.endswith(".flac"):
                                ...
                                labels.append(speaker_dir)  # 使用说话人ID作为标签
            ...
            for file in os.listdir(root):
                if file.endswith(".wav"):
                    ...
                    speaker_id = file.split("-")[0]  # 文件名格式: 03-01-01-01-01-01-01.wav
                    audio_paths.append(full_path)
                    labels.append(speaker_id)
    ...
    def _validate_dataset(self):
        ...
        for i in range(len(self.audio_paths) - 1, -1, -1):
            ...
            try:
                ...
                # 检查音频长度
                if len(signal) < Config.SAMPLE_RATE // 2:  # 小于0.5秒
    ...
    def __getitem__(self, idx):
        ...
        # 确保音频长度正确
        if len(signal) > Config.MAX_SAMPLES:  # 16000
            # 改为固定裁剪开头部分（确保一致性）
        ...
        # 添加噪声
        if self.add_noise and self.split == "train":  # 只在训练时添加噪声
    ...
    try:
        ...
        if "train" in dataloaders and len(dataloaders["train"].dataset) > 0:
            ...
            try:
                ...
                plt.title(f"音频波形 (标签: {label})")
                plt.xlabel("采样点")
                plt.ylabel("振幅")
                plt.tight_layout()
                plt.savefig("audio_sample.png")
                print("保存音频样本波形图为 audio_sample.png")
            except ImportError:
                print("无法导入matplotlib，跳过波形图生成")
        else:
            print("训练集为空，无法获取样本")
    except Exception as e:
