...
# 导入模块（假设在同一目录下）
try:
    from With_Lorzen_data_loader import SpeakerRecognitionDataset, get_dataloaders
    from With_Lorzen_c_hilap_model import CHiLAPModel
except ImportError as e:
    print(f"导入错误: {e}")
    print("请确保所有模块文件在同一目录下")


# 配置参数 - 针对T1模型（简化模型+复杂混沌模块）调整
class Config:
    # 训练参数
    EPOCHS = 50  # 增加训练轮数，因为复杂模块可能需要更长时间收敛
    LR = 0.0005  # 降低学习率，复杂模块需要更稳定的训练
    ...
    GRAD_CLIP = 1.0  # 梯度裁剪阈值（新增，防止复杂模块梯度爆炸）
    ...
    BATCH_SIZE = 8  # 减小批次大小，复杂模块需要更多内存
