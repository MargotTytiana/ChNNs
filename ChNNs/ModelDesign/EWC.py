...
class ElasticWeightConsolidation:
    ...
    def compute_fisher_matrix(self, dataset, num_samples, batch_size):
        ...
        for name in self.fisher:
            self.fisher[name] /= total_batches
    ...
    def penalty(self, model):
        ...
        for name, param in model.named_parameters():
            if name in self.params:
                ...
                # 加权平方差 (Fisher信息作为权重)
                loss += (self.fisher[name] * param_diff.pow(2)).sum()
        return loss

    def consolidate(self, model, optimizer, lambda_ewc=1000.0):
        """
        应用EWC正则化到优化过程
    ...
    def visualize_fisher_information(self, top_k=20):
        """
        可视化最重要的参数 (用于调试和分析)
        """
        if not self.fisher:
        ...
        print("\n=== 参数重要性排名 (Top {} 基于Fisher信息) ===".format(top_k))
        print("{:<50} {:<15}".format("参数名称", "重要性分数"))
        print("-" * 65)
        for i, (name, score) in enumerate(fisher_scores[:top_k]):
            print("{:<50} {:<15.6f}".format(name, score))
    ...
    @classmethod
    def load(cls, path, model, device):
        """从文件加载EWC状态"""
