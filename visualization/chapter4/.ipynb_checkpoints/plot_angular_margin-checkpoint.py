...
def extract_real_embeddings(data_dir, num_speakers=10, samples_per_speaker=20):
    ...
    Args:
        data_dir: Path to LibriSpeech dataset
        num_speakers: Number of speakers to visualize
        samples_per_speaker: Samples per speaker
    
    Returns:
        embeddings: numpy array of shape (N, embedding_dim)
        labels: numpy array of speaker labels
        prototypes: numpy array of class prototypes
    """
    ...
    config['num_speakers'] = actual_num_speakers
...
def generate_synthetic_embeddings(num_speakers=5, samples_per_speaker=30, embedding_dim=3):
    ...
    Args:
        ...
        embedding_dim: Dimension of embeddings (3 for visualization)
    
    Returns:
        embeddings: numpy array of shape (N, 3)
        labels: numpy array of speaker labels  
        prototypes: numpy array of class prototypes
    """
    np.random.seed(42)
    ...
    for i in range(num_speakers):
        ...
        phi = np.pi / 2 + 0.3 * np.sin(theta)  # Slight variation
    ...
    for speaker_id, proto in enumerate(prototypes):
        # Generate samples with some spread around prototype
...
def project_to_3d(embeddings, method='pca'):
    ...
    Returns:
        embeddings_3d: numpy array of shape (N, 3)
    """
...
def draw_decision_boundary(ax, proto1, proto2, margin=0.0, color='gray', alpha=0.3):
    ...
    Args:
        ax: 3D axes
        proto1, proto2: Two prototype vectors
        margin: Angular margin in radians
        color: Boundary color
        alpha: Transparency
    """
    # Normal to the plane containing both prototypes
    normal = np.cross(proto1, proto2)
    normal = normal / np.linalg.norm(normal)
    
    # If margin > 0, shift the boundary toward proto1
    if margin > 0:
        # Rotate normal slightly toward proto2
        axis = np.cross(normal, proto1)
        if np.linalg.norm(axis) > 1e-6:
            axis = axis / np.linalg.norm(axis)
            # Shift by margin angle
            c, s = np.cos(margin), np.sin(margin)
            rotation = np.array([
...
def plot_angular_margin_visualization(embeddings, labels, prototypes, 
    ...
    Args:
        embeddings: Speaker embeddings on unit sphere
    ...
    # Plot embeddings
    for speaker_id in range(min(num_speakers, 5)):  # Limit to 5 speakers for clarity
        mask = labels == speaker_id
    ...
    # Draw decision boundaries (with margin)
    for i in range(min(len(prototypes_3d)-1, 4)):
        for j in range(i+1, min(len(prototypes_3d), 5)):
...
def main():
    ...
    parser.add_argument('--data_dir', type=str, default=None,
                       help='Path to LibriSpeech dataset (optional, uses synthetic data if not provided)')
