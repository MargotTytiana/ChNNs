...
def plot_speaker_attractors():
    ...
    for spk in speakers:
        ...
        ax1.plot(traj[:, 0], traj[:, 1], traj[:, 2], 
                 ...
                 label=f'{spk["name"]} ($\\rho$={spk["rho"]:.1f})')
    ...
    ax2.set_ylabel('Y')
    ax2.set_zlabel('Z')
    ax2.set_title('Chaotic Trajectory ($\\rho$=28.0)')
...
def plot_bifurcation_effect():
    ...
    plt.suptitle('Effect of Bifurcation Parameter $\\rho$ on Attractor Geometry', 
                 fontsize=14, y=1.02)
    plt.tight_layout()
...
def plot_ablation_results():
    ...
    colors_test = ['#4682b4', '#7fa28f', '#b8a467', '#b5697b', '#998b9b']
    colors_val = ['#d0dfe6', '#b9dec9', '#ffeaab', '#f2b9b2', '#d1c2d3']
    
    bars1 = ax.bar(x - width/2, test_acc, width, label='Test Accuracy',
                   color=colors_test, edgecolor='black', linewidth=0.1)
    bars2 = ax.bar(x + width/2, val_acc, width, label='Val Accuracy',
                   color=colors_val, edgecolor='black', linewidth=0.1)
    ...
    ax.annotate('Best: 97.46%', xy=(3, 97.46), xytext=(3.28, 89),
                arrowprops=dict(arrowstyle='->', color='#863020'),
                fontsize=10, color='#863020', fontweight='bold')
...
def plot_regime_distribution():
    ...
    bp = ax2.boxplot(data_for_boxplot, tick_labels=[f'Spk {s+1}' for s in selected_speakers],
                     patch_artist=True)
    ...
    ax2_twin = ax2.twinx()
    ax2_twin.set_ylim(0.8 * 28, 1.2 * 28)  # $\\rho$ range after modulation
    ax2_twin.set_ylabel('Effective $\\rho$ (after modulation)')
